...
data class HttpActions(val env: String = "local") : ZettaiActions {
    ...
    private fun submitToZettai(
        path: String,
        webForm: Form,
    ): Response = client(log(Request(Method.POST, "http://localhost:$zettaiPort/$path").body(webForm.toBody())))
    ...
    override fun ToDoListOwner.`starts with a list`(
        ...
        fetcher.assignListToUser(
            user,
            ToDoList(ListName.fromTrusted(listName), items.map { ToDoItem(it) }),
        )
    }
